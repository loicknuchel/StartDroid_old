...
public class V0_KeyValueHelper {
    ...
    private V0_Bdd v0_Bdd;
    private SQLiteDatabase mDb;

    public V0_KeyValueHelper(final Context context) {
	v0_Bdd = new V0_Bdd(context, DbConstants.NOM_BDD, null,
		DbConstants.VERSION_BDD);
    }

    public void open() {
	mDb = v0_Bdd.getWritableDatabase();
    }
	...
	values.put(keyCol, val.getKey());
	values.put(valueCol, val.getValue());
	return mDb.update(dbTable, values, idCol + " = " + val.getId(), null);
    }

    public KeyValue get(final long id) {
	Cursor c = mDb.query(dbTable, new String[] { idCol, keyCol, valueCol },
	...
	Cursor c = mDb.query(dbTable, new String[] { idCol, keyCol, valueCol },
		keyCol + " LIKE \"" + key + "\"", null, null, null, null);
	...
	Cursor c = mDb.query(dbTable, new String[] { idCol, keyCol, valueCol },
		valueCol + " LIKE \"" + value + "\"", null, null, null, null);
